...
struct Validator {
    pub(crate) snakifier: Converter,
}

impl Default for Validator {
    fn default() -> Self {
        Self {
            snakifier: Converter::new()
...
impl Validator {
    pub fn validate(&self, spec: &OpenAPI) -> Vec<String> {
        let schema = spec.walk().flat_map(|(name, schema)| {
            self.validate_subschemas(spec, schema)
                .map(|msg| {
                    format!(
                        "problem with type {}: {}",
...
mod tests {
    ...
    fn test_ipv6() {
        ...
        assert_eq!(validator.snakifier.convert("ipv6"), "ipv6");
        assert_eq!(
            validator.snakifier.convert("the_ipv6_network"),
            "the_ipv6_network"
        );
    }
}
